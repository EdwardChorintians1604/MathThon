...
def submit_register_user():
    ...
    try:
        if photo and photo.filename.strip() != "":
            ...
            frontend_static_dir = os.path.join(current_app.static_folder, 'uploads')
            os.makedirs(frontend_static_dir, exist_ok=True)
            foto_path = os.path.join(frontend_static_dir, unique_filename)
            photo.save(foto_path)
            foto_data = f'uploads/{unique_filename}'  # Path relatif untuk database (relatif ke /static)
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if user and check_password_hash(user['password'], password):
                ...
                return redirect(url_for('dashboard_user')) # This route is outside the blueprint
            else:
...
def google_login():
    ...
    try:
        ...
        return jsonify({
            ...
            "redirect": url_for('dashboard_user')
        }), 200
        ...
        logging.error(f"General error during Google auth: {e}")
